#!/bin/bash
eg=$1
benchmarkmodel=$2
testmodel=$3
set -e
if [ -z ${RMG_BENCHMARK+x} ]; then 
	echo "RMG variable is unset. Exiting..."
	exit 0
fi

export ORIGIN_PYTHONPATH=$PYTHONPATH
echo "Running $1 example"

###########
#BENCHMARK#
###########
# make folder for models generated by the benchmark version of RMG-Py/RMG-database:
mkdir -p tests/$benchmarkmodel/$eg
rm -rf tests/$benchmarkmodel/$eg/*
cp examples/rmg/$eg/input.py tests/$benchmarkmodel/$eg/input.py

source activate benchmark

echo "benchmark version of RMG: "$RMG_BENCHMARK
export PYTHONPATH=$RMG_BENCHMARK:$ORIGIN_PYTHONPATH 

# use the rmgrc file to point to the location of the desired RMG-database:
rmgrc="database.directory : "${RMGDB_BENCHMARK}/input/
rm -rf $HOME/.rmg
mkdir -p $HOME/.rmg
echo $rmgrc >> $HOME/.rmg/rmgrc

python $RMG_BENCHMARK/rmg.py tests/$benchmarkmodel/$eg/input.py > /dev/null

source deactivate
export PYTHONPATH=$ORIGIN_PYTHONPATH

#########
#TESTING#
#########
# make folder for models generated by the test version of RMG-Py and RMG-database:
mkdir -p tests/$testmodel/$eg
rm -rf tests/$testmodel/$eg/*
cp examples/rmg/$eg/input.py tests/$testmodel/$eg/input.py
source activate testing
echo "test version of RMG: "$RMG_TESTING

export PYTHONPATH=$RMG_TESTING:$ORIGIN_PYTHONPATH 

# use the rmgrc file to point to the location of the desired RMG-database:
rmgrc="database.directory : "${RMGDB_TESTING}/input/
rm -rf $HOME/.rmg
mkdir -p $HOME/.rmg
echo $rmgrc >> $HOME/.rmg/rmgrc

python $RMG_TESTING/rmg.py tests/$testmodel/$eg/input.py > /dev/null
export PYTHONPATH=$ORIGIN_PYTHONPATH
source deactivate


# compare both generated models
source activate benchmark
export PYTHONPATH=$RMG_BENCHMARK:$ORIGIN_PYTHONPATH 

bash check.sh $eg tests/$benchmarkmodel/$eg tests/$testmodel/$eg

export PYTHONPATH=$ORIGIN_PYTHONPATH
source deactivate

# make folder for models generated by the test version of RMG-Py and RMG-database, with scoop enabled:
mkdir -p tests/$testmodel/$eg/scoop
rm -rf tests/$testmodel/$eg/scoop/*
cp examples/rmg/$eg/input.py tests/$testmodel/$eg/scoop/input.py
echo "Version of RMG running with SCOOP: $RMG"
source activate testing
export PYTHONPATH=$RMG_TESTING:$ORIGIN_PYTHONPATH

python -m scoop -n 1 $RMG_TESTING/rmg.py tests/$testmodel/$eg/scoop/input.py > /dev/null

export PYTHONPATH=$ORIGIN_PYTHONPATH
source deactivate

# compare both generated models
source activate benchmark
export PYTHONPATH=$RMG_BENCHMARK:$ORIGIN_PYTHONPATH 

bash check.sh $eg tests/$benchmarkmodel/$eg tests/$testmodel/$eg/scoop

export PYTHONPATH=$ORIGIN_PYTHONPATH
source deactivate